...
async function main() {
  ...
  const bal = async (label: string) => {
    const bMe = await publicClient.readContract({ address: TOKEN, abi, functionName: "balanceOf", args: [me] }) as bigint;
    const bYou = await publicClient.readContract({ address: TOKEN, abi, functionName: "balanceOf", args: [you] }) as bigint;
  ...
  const tx1 = await wallet.writeContract({
    ...
    args: [you, parseUnits("100", 18)],   // ✅ fixed
  ...
  const tx2 = await wallet.writeContract({
    ...
    args: [you, parseUnits("50", 18)],   // ✅ fixed
